...
legend('Reference Velocity',  'Location', 'best');
...
function res = simulate_single_segment(mode, t_vec, dt, x0, x1, v_max, acc_a, acc_d, acc_trap, K, M, K_s, C)
    ...
    switch lower(mode)
        case 's_trapezoid_variable_acc'
            [vp, Tja, Tjd, Ta, Tc, Td, Ttot] = ...
                calc_asym_logic_cp_axis_single(S_total, v_max, acc_a, acc_d, vs, ve);
    ...
    for i = 1:N_steps
        ...
        switch lower(mode)
            case 's_trapezoid_variable_acc'
                [v_ref_abs, a_ref_abs] = eval_scurve_at_time_single( ...
...
% =========================================================================
% 与 numberTran9_2 一致的 S 梯形变加速参数计算
% 对应 calc_asym_logic_cp_axis(S, vmax, aa, ad, vs, ve)
% =========================================================================
...
% =========================================================================
% 与 numberTran9_2 一致的 S 梯形变加速速度/加速度求值
% 对应 eval_scurve_at_time(...)
% =========================================================================
function [v, a] = eval_scurve_at_time_single(t, vs, ve, vp, aa, ad, Tja, Tjd, Ta, Tc, Td, Ttot)
    ...
    elseif t < Ta
        % 第3段：减加速度段，a 从 +aa 线性下降到 0
        d_dt_acc = Ta - t;
        a = (aa / max(Tja, 1e-9)) * d_dt_acc;
        v = vp - 0.5 * (aa / max(Tja, 1e-9)) * d_dt_acc^2;
    ...
    elseif t < (Ta + Tc + 2*Tjd)
        ...
        a = -ad;
        v_start_dec = vp - 0.5 * ad * Tjd;
        v = v_start_dec - ad * d_dt_dec;

    else
        % 第7段：减减速度段，a 从 -ad 线性回升到 0
        d_dt_end = Ttot - t;
        if d_dt_end < 0
            d_dt_end = 0;
        end
        a = -(ad / max(Tjd, 1e-9)) * d_dt_end;
        v = ve + 0.5 * (ad / max(Tjd, 1e-9)) * d_dt_end^2;
    end
end
% =========================================================================
% 普通梯形：单段参数计算
