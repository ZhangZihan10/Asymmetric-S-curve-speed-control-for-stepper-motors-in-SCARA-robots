function EE_traj = numberTran9_2_Plot2(path)
    % ==========================================================
    ...
    for k = 1:num_segments
        ...
        for j = 1:3
            ...
            vs = V_node_axis(k,   j); ve = V_node_axis(k+1, j);
    ...
    %% 8. 构建机械臂模型与正运动学解算 (Forward Kinematics)
    disp('正在加载 Robotics Toolbox 机械臂参数进行正运动学解算...');
    ...
    % ★ 新增：在同步后的曲线上标注各段的起始节点速度 (空心圆/方块/三角)
    plot(t_node, V_node_sync(:,1), 'ro', 'MarkerSize', 5, 'MarkerFaceColor', 'w', 'LineWidth', 1.2);
    plot(t_node, V_node_sync(:,2), 'gs', 'MarkerSize', 5, 'MarkerFaceColor', 'w', 'LineWidth', 1.2);
    plot(t_node, V_node_sync(:,3), 'b^', 'MarkerSize', 5, 'MarkerFaceColor', 'w', 'LineWidth', 1.2);
    
    grid on; title('(a) Time-Synchronized S-Curve Velocity Profile');
    ...
    % --- 注：已按要求取消绘制末端轨迹图（Figure 4），直接作为函数返回值输出 ---
    
    disp('仿真计算与绘图完成！末端轨迹数据已输出。');
end
