function numberTran6(arduino, path)
...
% path: 避障算法算出的 N x 4 矩阵 [Z(m), Y(rad), X(rad), T(rad)]
    ...
    % 3. 多轴时间同步优化算法 (极速拉扯)
    % ==========================================================
    V_MAX_LIMITS = [6000, 5000, 5000]; % 保护性钳位极限
    V_sync = zeros(num_points, 3);
    
    % ★ 新增：用于记录绝对时间轴的数组
    t_history = zeros(num_points, 1); 
    ...
    for k = 1:(num_points - 1)
        ...
        % 累加时间，生成真实的时间戳
        t_history(k+1) = t_history(k) + T_sync;
    ...
    % ==========================================================
    % ★ 新增：绘制 SCI 学术级别动力学曲线图 ★
    % ==========================================================
    % 计算脉冲域加速度 (使用数值梯度)
    A_sync = zeros(num_points, 3);
    for j = 1:3
        A_sync(:, j) = gradient(V_sync(:, j)) ./ gradient(t_history);
    end
    
    figure('Name', 'Multi-Axis Synchronization Kinematics', 'Position', [100, 100, 1000, 700], 'Color', 'w');
    
    % 子图 (a)：同步速度曲线
    subplot(2,1,1);
    % 为了真实反映阶跃下发特性，这里使用点线结合
    plot(t_history, V_sync(:,1), 'r-o', 'LineWidth', 1.5, 'MarkerSize', 4); hold on;
    plot(t_history, V_sync(:,2), 'g-s', 'LineWidth', 1.5, 'MarkerSize', 4);
    plot(t_history, V_sync(:,3), 'b-^', 'LineWidth', 1.5, 'MarkerSize', 4);
    grid on;
    title('(a) Time-Synchronized Velocity Profile in Pulse Domain');
    xlabel('Time (s)');
    ...
    % 子图 (b)：离散加速度曲线
    subplot(2,1,2);
    plot(t_history, A_sync(:,1), 'r-', 'LineWidth', 1.2); hold on;
    plot(t_history, A_sync(:,2), 'g-', 'LineWidth', 1.2);
    plot(t_history, A_sync(:,3), 'b-', 'LineWidth', 1.2);
    grid on;
    title('(b) Implied Acceleration Profile');
    xlabel('Time (s)');
