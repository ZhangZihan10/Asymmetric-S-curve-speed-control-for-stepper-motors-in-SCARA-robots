function Baseline_Trapezoidal_Plot(path)
    % =====================================================================
    % [对比基线算法] 传统独立梯形加减速模拟 (对应旧版 AccelStepper 逻辑)
    % Inputs: 
    %   arduino: 兼容输入接口 (此脚本主要用于绘图对比，不实际下发执行)
    ...
    % 这里不再有非对称减速，AccelStepper 默认 A = D
    ...
    for k = 1:(num_points - 1)
        % 计算当前微小线段的三轴绝对脉冲距离
        delta_Z = abs(motor_pos(k+1, 1) - motor_pos(k, 1));
        delta_Y = abs(motor_pos(k+1, 2) - motor_pos(k, 2));
        delta_X = abs(motor_pos(k+1, 3) - motor_pos(k, 3));
        
        % 分别计算每个轴独立跑完所需的时间
        T_Z = calc_independent_time(delta_Z, Vmax_Z, Amax_Z);
        T_Y = calc_independent_time(delta_Y, Vmax_Y, Amax_Y);
        T_X = calc_independent_time(delta_X, Vmax_X, Amax_X);
        ...
        % 计算各自的梯形速度与加速度曲线
        [v_z, a_z] = get_trapz_state(t_array, delta_Z, Vmax_Z, Amax_Z);
        [v_y, a_y] = get_trapz_state(t_array, delta_Y, Vmax_Y, Amax_Y);
        [v_x, a_x] = get_trapz_state(t_array, delta_X, Vmax_X, Amax_X);
    ...
    legend('J1', 'J2', 'J3', 'Location', 'northeast');
